...
public class Interpreter {
  ...
  public static Tuple interpretStatement(String input, DependencyManager depman)
    ...
    Pair<String, String> parserAndProgram = null;
    ...
    Tuple parser = parserName.equals("__BUILTIN__")
        ? null
        : interpretStatement(depman.getResource(parserName), depman);

    Tuple mainProgExp = Lambda.car(Lambda.app(parser, Lambda.data(LSymbol
        .of(programBody))));
    Tuple depsListExp = Lambda.cdr(Lambda.app(parser, Lambda.data(LSymbol
        .of(programBody))));

    List<Object> depsList = (List<Object>) Util.convertPrimitiveToJava(Lambda
        .reduce(depsListExp));
